...
void WaypointAdd (int wpt_type)
{
   ...
   float TempDistance[MAX_PATH_INDEX];
   ...
   else // KWo - 09.11.2007 - rewritten code to prevent making stupid connections
   {
      // calculate all the paths to this new waypoint
      j = 0;
      ...
      for (i = 0; i < g_iNumWaypoints; i++) // this loop is to find 8 closests waypoints...
      {
         if (i == index)
            continue; // skip the waypoint that was just added
            ...
            else
            {
               if ((bLongestCalculated) && (iPathIndexOfLongest > -1)) // we are trying to find 8 closest waypoints to connect
               {
                  if (distance < fLongestDist)
                  {
                     ...
                     TempDistance[iPathIndexOfLongest] = distance;
               ...
               else
               {
                  ...
                  for (k = 0; k < MAX_PATH_INDEX; k++)
                  {
                     if (TempDistance[k] > fLongestDist)
                     {
                        fLongestDist = TempDistance[k];
                        iPathIndexOfLongest = k;
                        bLongestCalculated = TRUE;
                     }
                  }
                  if ((bLongestCalculated) && (iPathIndexOfLongest > -1))
                  {
                     if (distance < fLongestDist)
                     {
                        fLongestDist = 0.0;
                        TempInd[iPathIndexOfLongest] = i; // here we are replacing the longest path by any closer one...
                        TempDistance[iPathIndexOfLongest] = distance;
                        bLongestCalculated = FALSE;
                        j++;
                     }
...
testagain:
         ...
         {
            if (1.1 * (TempDistance[k] + TempDistance[k - 2])/2.0 < TempDistance[k - 1])
            {
//               ALERT(at_logged,"[DEBUG] Prevent making a stupid connection from index = %d to %d.\n", index, TempInd[k - 1]);
      ...
      if ((Temp_ind > -1) && (TempInd[0] > -1) && (TempInd[1] > -1))
      {
         ...
         {
            if (1.1 * (TempDistance[1] + Temp_dist)/2.0 < TempDistance[0])
            {
//               ALERT(at_logged,"[DEBUG] Prevent making a stupid connection from index = %d to %d.\n", index, TempInd[0]);
      ...
      for (k = 0; k < MAX_PATH_INDEX; k++)
      {
         ...
         if (i > -1)
         {
            distance = TempDistance[k];
            WaypointAddPath (index, i, distance);
//            ALERT(at_logged,"[DEBUG] - AddWaypoint - index = %i, distance = %i, angle = %i.\n",
//               i, int(distance), int(TempAngleY[k]));
...
//   int flags;
   ...
   for (k = 0; k < MAX_PATH_INDEX; k++)
   {
      ...
      Temp_ind = TempInd[k];
      if ((Temp_ind > -1) && (Temp_ind < g_iNumWaypoints))
      {
         vec_dir_ang = UTIL_VecToAngles(paths[Temp_ind]->origin - paths[index]->origin);
         TempAngleY[k] = vec_dir_ang.y;
         UTIL_ClampAngle(&TempAngleY[k]);
         TempDistance[k] = float (paths[index]->distance[k]);
         TempPathNr[k] = k;
//         ALERT(at_logged,"[DEBUG] Wp Clean - index %d connected to %d [%d] with angle = %d and distance = %d.\n",
//            index, Temp_ind, k, int(TempAngleY[k]), int(TempDistance[k]));
      }
   }
      ...
      if (TempAngleY[k] - TempAngleY[k - 2] < 80.0)
      {
         if (1.1 * (TempDistance[k] + TempDistance[k - 2])/2.0 < TempDistance[k - 1])
         {
            if ((paths[index]->index[TempPathNr[k - 1]] == TempInd[k - 1])
                  && !(paths[index]->connectflag[TempPathNr[k - 1]] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
            {
//               ALERT(at_logged,"[DEBUG] Removing a stupid connection from index = %d to %d.\n", index, TempInd[k - 1]);
               ...
               for(j = 0; j < MAX_PATH_INDEX; j++)
               {
                  if ((paths[TempInd[k - 1]]->index[j] == index)
                        && !(paths[TempInd[k - 1]]->connectflag[j] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
                  {
//                     ALERT(at_logged,"[DEBUG] Removing also a stupid connection from index = %d to %d.\n", TempInd[k - 1], index);
            ...
            {
//               ALERT(at_logged,"[DEBUG] Failed to remove a stupid connection from index = %d to %d.\n", index, TempInd[k - 1]);
               return;
            }
         }
      }
      ...
      if ((TempAngleY[1] - Temp_ang < 80.0) || (360.0 - (TempAngleY[1] - Temp_ang) < 80.0))
      {
         if (1.1 * (TempDistance[1] + Temp_dist)/2.0 < TempDistance[0])
         {
            if ((paths[index]->index[TempPathNr[0]] == TempInd[0])
                  && !(paths[index]->connectflag[TempPathNr[0]] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
            {
//               ALERT(at_logged,"[DEBUG] Removing (2) a stupid connection from index = %d to %d.\n", index, TempInd[0]);
               ...
               for(j = 0; j < MAX_PATH_INDEX; j++)
               {
                  if ((paths[TempInd[0]]->index[j] == index)
                        && !(paths[TempInd[0]]->connectflag[j] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
                  {
//                     ALERT(at_logged,"[DEBUG] Removing (2) also a stupid connection from index = %d to %d.\n", TempInd[0], index);
            ...
            {
//               ALERT(at_logged,"[DEBUG] Failed to remove (2) a stupid connection from index = %d to %d.\n", TempInd[0], index);
               return;
            }
         }
      }
...
testagain2:
      ...
      {
         if ((TempDistance[k] > 1.1 * TempDistance[k - 1]))
         {
            if ((paths[index]->index[TempPathNr[k]] == TempInd[k])
                  && !(paths[index]->connectflag[TempPathNr[k]] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
            {
//               ALERT(at_logged,"[DEBUG] Removing (3) a stupid connection from index = %d to %d.\n", index, TempInd[k]);
               ...
               for(j = 0; j < MAX_PATH_INDEX; j++)
               {
                  if ((paths[TempInd[k]]->index[j] == index)
                        && !(paths[TempInd[k]]->connectflag[j] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
                  {
//                     ALERT(at_logged,"[DEBUG] Removing (3) also a stupid connection from index = %d to %d.\n", TempInd[k], index);
            ...
            {
//               ALERT(at_logged,"[DEBUG] Failed to remove (3) a stupid connection from index = %d to %d.\n", index, TempInd[k]);
               return;
            }
         }
         else if ((TempDistance[k - 1] > 1.1 * TempDistance[k]))
         {
            if ((paths[index]->index[TempPathNr[k - 1]] == TempInd[k - 1])
                  && !(paths[index]->connectflag[TempPathNr[k - 1]] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
            {
//               ALERT(at_logged,"[DEBUG] Removing (4) a stupid connection from index = %d to %d.\n", index, TempInd[k - 1]);
               paths[index]->index[TempPathNr[k - 1]] = -1; // unassign this path
               paths[index]->connectflag[TempPathNr[k - 1]] = 0;
               paths[index]->vecConnectVel[TempPathNr[k - 1]] = g_vecZero;
               paths[index]->distance[TempPathNr[k - 1]] = 0;
               ...
               for(j = 0; j < MAX_PATH_INDEX; j++)
               {
                  if ((paths[TempInd[k - 1]]->index[j] == index)
                        && !(paths[TempInd[k - 1]]->connectflag[j] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
                  {
//                     ALERT(at_logged,"[DEBUG] Removing (4) also a stupid connection from index = %d to %d.\n", TempInd[k - 1], index);
                     paths[TempInd[k - 1]]->index[j] = -1; // unassign this path
                     paths[TempInd[k - 1]]->connectflag[j] = 0;
                     paths[TempInd[k - 1]]->vecConnectVel[j] = g_vecZero;
                     paths[TempInd[k - 1]]->distance[j] = 0;
            ...
            {
//               ALERT(at_logged,"[DEBUG] Failed to remove (4) a stupid connection from index = %d to %d.\n", index, TempInd[k -1]);
               return;
            }
         }
         else
...
/*
   for (k = 0; k < MAX_PATH_INDEX; k++)
   {
      if (TempInd[k] > -1)
      {
         ALERT(at_logged,"[DEBUG] - WP Clean - we still have connection between index = %d and %d [%d] with angle = %d and distance = %d.\n",
...
*/
   if ((Temp_ind > -1) && (TempInd[0] > -1))
   {
      ...
      {
         if (1.1 * Temp_dist < TempDistance[0])
         {
            if ((paths[index]->index[TempPathNr[0]] == TempInd[0])
                  && !(paths[index]->connectflag[TempPathNr[0]] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
            {
//               ALERT(at_logged,"[DEBUG] Removing (5) a stupid connection from index = %d to %d.\n", index, TempInd[0]);
               paths[index]->index[TempPathNr[0]] = -1; // unassign this path
               paths[index]->connectflag[TempPathNr[0]] = 0;
               paths[index]->vecConnectVel[TempPathNr[0]] = g_vecZero;
               paths[index]->distance[TempPathNr[0]] = 0;
               ...
               for(j = 0; j < MAX_PATH_INDEX; j++)
               {
                  if ((paths[TempInd[0]]->index[j] == index)
                        && !(paths[TempInd[0]]->connectflag[j] & C_FL_JUMP)) // KWo - 24.01.2010 - don't remove jump connections
                  {
//                     ALERT(at_logged,"[DEBUG] Removing (5) also a stupid connection from index = %d to %d.\n", TempInd[0], index);
                     paths[TempInd[0]]->index[j] = -1; // unassign this path
                     paths[TempInd[0]]->connectflag[j] = 0;
                     paths[TempInd[0]]->vecConnectVel[j] = g_vecZero;
                     paths[TempInd[0]]->distance[j] = 0;
            ...
            {
//               ALERT(at_logged,"[DEBUG] Failed to remove (5) a stupid connection from index = %d to %d.\n", TempInd[0], index);
               return;
            }
         }
      }
