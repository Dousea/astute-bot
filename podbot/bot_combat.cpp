...
/*
            if (pHostEdict)
            {
               if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                  ALERT (at_logged,"[DEBUG] Bot %s cannot fire because it may hurt its friend %s (1).\n", pBot->name, STRING (clients[i].pEdict->v.netname));
            }
*/
            return (TRUE);
         }
...
/*
            if (pHostEdict)
            {
               if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
               {
                  ALERT (at_logged,"[DEBUG] Bot %s cannot fire because it may hurt its friend %s (2), fConeDev = %f, fConeMin = %f, fTeamnateDist = %f, fDistance = %f.\n",
...
//         ALERT (at_logged, "[DEBUG] Bot %s is asked to don't move because of the fightstyle; time = %f.\n", pBot->name, gpGlobals->time);
...
//      ALERT(at_logged,"[DEBUG] Bot %s - to far away to throw a nade - distance = %.0f.\n", pBot->name, dist3);
