...
//   ALERT(at_logged, "[DEBUG] RateGroundWeapon - Bot %s has %d and found %d.\n", pBot->name,
//      iHasWeapon, iGroundIndex);
...
/*
         if (pHostEdict)
         {
            if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
               ALERT(at_logged, "[Debug] Bot %s found a breakable from position %i ; wpt_index = %i.\n", pBot->name, i, pBot->curr_wpt_index);
         }
*/
...
*/
   ...
   while (distance > 30.0) // KWo - 30.10.2009
   {
      ...
      {
//         UTIL_ServerPrint ("[Debug] Bot %s can fall down from tested location v_check = [%i, %i, %i].\n", pBot->name, (int) v_check.x, (int) v_check.y, (int) v_check.z);
         return (TRUE);
      }
...
//         ALERT(at_logged,"[DEBUG] Bot %s still can take care about found item %s.\n", pBot->name, g_ItemTypeNames[pBot->iPickupType]);
   ...
   while (!FNullEnt (pent = FIND_ENTITY_IN_SPHERE (pent, pEdict->v.origin, 500)))
   {
      ...
      {
         bBomb = TRUE;
//         distance = (vecEntityOrigin - pEdict->v.origin).Length ();
...
//         UTIL_ServerPrint("[DEBUG] Bot %s has SEEKCOVER's fDesisre = %f; ENEMYHUNT's fDesire = %f.\n", pBot->name, fRetreatLevel * fRatio, fLevel);
   }
   else
   {
...
//            ALERT(at_logged, "[DEBUG] BotCheckTaskPri - Bot %s deleted the previous task %i (couldn't be continued); task deep = %i.\n", pBot->name, iTask, pBot->i_TaskDeep);
         }
         pBot->pTasks = pNextTask;
      }
   }
   ...
   if (pBot->pTasks != NULL) // it shouldn't happen this condition now as false...
   {
      if (pBot->pTasks->iTask == pTask->iTask)
      {
         if (pBot->pTasks->iData != pTask->iData)
         {
            ...
            DeleteSearchNodes(pBot);
            pBot->pTasks->iData = pTask->iData;
//            ALERT(at_logged, "[DEBUG] BotPushTask - Bot %s got the new-old task %i with different goal; task deep = %i.\n", pBot->name, pTask->iTask, pBot->i_TaskDeep);
         }
         if (pBot->pTasks->fDesire != pTask->fDesire)
         {
            pBot->pTasks->fDesire = pTask->fDesire;
            bCheckPriorities = TRUE;
//            ALERT(at_logged, "[DEBUG] BotPushTask - Bot %s got the new-old task %i with different desire; task deep = %i.\n", pBot->name, pTask->iTask, pBot->i_TaskDeep);
         }
...
//         else
//            ALERT(at_logged, "[DEBUG] BotPushTask - Bot %s got the new task %i already existing on the stack; task deep = %i.\n", pBot->name, pTask->iTask, pBot->i_TaskDeep);
      }
   }
...
//      ALERT(at_logged, "[DEBUG] BotPushTask - Bot %s got the new different task %i; task deep = %i.\n", pBot->name, pTask->iTask, pBot->i_TaskDeep);
   ...
   if (bCheckPriorities)
   {
      ...
      {
//         ALERT(at_logged, "[DEBUG] BotPushTask - Bot %s got other task %i with higher priority (after checking prior.); task deep = %i.\n", pBot->name, pTask->iTask, pBot->i_TaskDeep);
...
//         ALERT(at_logged, "[DEBUG] BotRemoveCertainTask - Bot %s removes the task %i; task deep %i.\n", pBot->name, iTaskNum, pBot->i_TaskDeep);
         break;
      }
...
//      ALERT(at_logged, "[DEBUG] BotTaskComplete - Bot %s got task NULL to finish; task deep = %i.\n", pBot->name, pBot->i_TaskDeep);
   ...
   if ((pPrevTask) && (pNextTask))
   {
      ...
      {
         pBot->pTasks = pPrevTask;
//         ALERT(at_logged, "[DEBUG] BotTaskComplete - Bot %s got the previous task = %i from the stack; task deep = %i.\n", pBot->name, pBot->pTasks->iTask, pBot->i_TaskDeep);
      }
      else
      {
         pBot->pTasks = pNextTask;
//         ALERT(at_logged, "[DEBUG] BotTaskComplete - Bot %s got the next task  = %i from the stack; task deep = %i.\n", pBot->name, pBot->pTasks->iTask, pBot->i_TaskDeep);
      }
   }
   else if (pPrevTask)
   {
...
//            pBot->name, STRING(pBot->pBotEnemy->v.netname), weapon_defs[iWeaponEnemy].szClassname, fEnemyDot, pEdict->v.weaponanim);
...
/*
         if ((BotUsesSniper (pBot))
            ...
            && (pBot->f_bot_see_enemy_time + 4.0 < gpGlobals->time)
            && (pBot->f_heard_sound_time + 4.0 < gpGlobals->time)) // KWo - 09.07.2008
         {

            i = HighestPistolOfEdict (pEdict);//ptrWeaponPrefs[pBot->bot_personality][GetBestSecondaryWeaponCarried (pBot)];
...
*/
      ...
      // calculate new path to Bomb Point if he's not already heading for
         ...
         // if the bot is about to come to the camp spot, but there is already camping someone else...
...
/*
         if (pHostEdict)
         {
            if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
               ALERT(at_logged, "[DEBUG] BotThink - TASK_NORMAL - Bot %s reached the goal.\n", pBot->name);
         }
*/
...
//                      ALERT (at_logged, "[DEBUG] Bot %s should start the task PLANT_BOMB.\n", pBot->name);
...
/*
            ...
            {
               if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
                  ALERT(at_logged, "[DEBUG] BotThink - TASK_NORMAL - Bot %s has no valid goal.\n", pBot->name);
            }
*/
            pBot->f_move_speed = 0.0;
...
//             ALERT(at_logged, "[DEBUG] Bot %s had no valid goal so now it has the new one - %i.\n", pBot->name, iDestIndex);
               if (iDestIndex != pBot->curr_wpt_index)
               {
//                ALERT(at_logged, "[DEBUG] Bot %s does the pathfinding from TASK_NORMAL.\n", pBot->name);
                  // Do Pathfinding if it's not the current waypoint
               ...
               && (g_i_botthink_index == g_iFrameCounter)) // KWo - 18.02.2008
         {
            ...
            if (paths[pBot->chosengoal_index]->flags & W_FL_RESCUE)
            {
               ...
               {
//                ALERT(at_logged,"[DEBUG] Bot %s had some hostages and was going to a rescue point, but they got killed.\n", pBot->name);
                  for (i = 0; i < MAX_HOSTAGES; i++)
                     pBot->pHostages[i] = NULL;
...
//          ALERT(at_logged, "[DEBUG] Bot %s has a C4 and maybe he wasn't going to plant it.\n", pBot->name);
            ...
            for (i = 0; i < g_iNumGoalPoints; i++)
            {
               ...
               {
//                ALERT(at_logged, "[DEBUG] Bot %s cannot see any group of enemies near the goal WP %d.\n", pBot->name, iGoalIndex);
            ...
            if (min_index > -1)
            {
               ...
               {
//               ALERT(at_logged, "[DEBUG] Bot %s chose the closest bombsite - previous goal %d.\n", pBot->name, pBot->chosengoal_index);
                  DeleteSearchNodes (pBot);
                  BotGetSafeTask(pBot)->iData = iGoalIndex;
                  pBot->chosengoal_index = iGoalIndex;
//                ALERT(at_logged, "[DEBUG] Bot %s chose the closest bombsite - goal %d.\n", pBot->name, iGoalIndex);
               }
            }
...
//                   ALERT(at_logged, "[DEBUG] Bot %s chose another bombsite.\n", pBot->name);
         ...
         // checks once a while if the bomb position isn't so close so we can see it and go there instead to go somewhere else...
               ...
               || (fabs(paths[pBot->chosengoal_index]->origin.z - g_vecBomb.z) > 40.0)) // KWo - 05.09.2008
            {
               ...
               {
//                ALERT(at_logged,"[DEBUG] Bot %s can see the bomb.\n", pBot->name);
                  min_index = -1;
                  ...
                  if (iGoalIndex > -1)
                  {
                     ...
                     BotPushTask (pBot, &TempTask);
//                  ALERT(at_logged, "[DEBUG] Bot %s chose another goal.\n", pBot->name);
                  }
               }
            }
         }
...
/*
         ...
         {
            if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
               ALERT(at_logged, "[DEBUG] TASK_SPRAYLOGO - Bot %s - taskend time = %.1f, global time = %.1f.\n",
...
//         ALERT(at_logged, "[DEBUG] BotThink - TASK_ENEMYHUNT - bot %s, time = %.2f.\n", pBot->name, gpGlobals->time);
/*
         if ((BotUsesSniper (pBot))
            && FNullEnt (pBot->pBotEnemy)
            && (pBot->f_bot_see_enemy_time + 4.0 < gpGlobals->time)
...
*/
            ...
            || FNullEnt (pBot->pHuntEnemy)) && (pBot->current_weapon.iId != CS_WEAPON_KNIFE))
         {
            ...
            bEndTask = true;
//          UTIL_ServerPrint("[DEBUG] Bot %s found a visible enemy or has no last enemy - finishing hunting.\n", pBot->name);
         }
         ...
         else if (BotDoWaypointNav (pBot))
         {
            ...
            if (BotGetSafeTask(pBot)->iTask == TASK_ENEMYHUNT) // KWo - 02.09.2006 - against BotRemoveCertainTask
            {
               ...
               pBot->prev_time = gpGlobals->time + 1.5; // KWo - 25.02.2008
               pBot->fNoCollTime = gpGlobals->time + 2.0;   // KWo - 02.01.2010
//              UTIL_ServerPrint("[DEBUG] Bot %s reached destination hunting enemy's position - finishing hunting.\n", pBot->name);
            }
            bEndTask = true;
         }
            ...
            /* && FNullEnt (pBot->pBotEnemy) */ && !FNullEnt (pBot->pHuntEnemy) && (pBot->vecHuntEnemyOrigin != g_vecZero))
         {
            ...
            {
               GetValidWaypoint (pBot);
//             UTIL_ServerPrint("[DEBUG] Bot %s was needing to find the current waypoint while hunting - nr %i.\n", pBot->name, pBot->curr_wpt_index);
            }
            pBot->pWayNodesStart = FindLeastCostPath (pBot, pBot->curr_wpt_index, iDestIndex);
            pBot->pWaypointNodes = pBot->pWayNodesStart;
            pBot->prev_time = gpGlobals->time + 1.5; // KWo - 25.02.2008
...
/*
         ...
         {
            if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
               ALERT(at_logged, "[DEBUG] TASK_ENEMYHUNT Bot %s needed to delete path nodes and now will be heading for WP nr %i.\n", pBot->name, iDestIndex);
         }
*/
         }
...
/*
         if (pBot->bIsReloading)
            ALERT(at_logged, "[DEBUG] TASK_SEEKCOVER - reloading bot %s.\n", pBot->name);
*/
...
/*
               if (pBot->bIsReloading)
                  ALERT(at_logged, "[DEBUG] TASK_SEEKCOVER - reloading bot %s searching for the goal.\n", pBot->name);
*/
               DeleteSearchNodes (pBot);
               pBot->prev_time = gpGlobals->time + 0.5; // KWo - 25.02.2008
               pBot->fNoCollTime = gpGlobals->time + 1.0;   // KWo - 02.01.2010
...
*/
         ...
         if (BotDoWaypointNav (pBot) || ((g_iDebugGoalIndex != -1) && (fDesire != TASKPRI_PLANTBOMB)))
         {
            ...
            {
//            ALERT(at_logged, "[DEBUG] Bot %s finished the task TASK_MOVETOPOSITION - WPTindex = %i; priority = %d.\n", pBot->name, pBot->curr_wpt_index, (int)fDesire);
               pBot->chosengoal_index = -1;
               pBot->prev_goal_index = -1;
               BotGetSafeTask(pBot)->iData = -1;
               BotTaskComplete(pBot);
               ...
               {
//                ALERT(at_logged, "[DEBUG] Bot %s forced to stay away from the ladder - WPTindex = %i and to pause a bit...\n", pBot->name, pBot->curr_wpt_index);
      ...
      case TASK_PLANTBOMB:
      {
         ...
         else if (pBot->b_bomb_blinking && (pEdict->v.weapons & (1 << CS_WEAPON_C4)))
         {
            if (pBot->current_weapon.iId == CS_WEAPON_C4)
            {
               ...
               pBot->f_move_speed = 0.0;
               pBot->f_sidemove_speed = 0.0;
//            ALERT (at_logged, "[DEBUG] Bot %s is planting C4.\n", pBot->name);
            }
            else if ((pBot->fTimeWeaponSwitch + 0.1 < gpGlobals->time) && (pBot->current_weapon.iId != CS_WEAPON_INSWITCH))
            {
               ...
               pBot->f_sidemove_speed = 0.0;
               SelectWeaponByName (pBot, "weapon_c4");
//             ALERT (at_logged, "[DEBUG] Bot %s is selecting C4.\n", pBot->name);
            }
         }
...
//                pBot->f_wpt_timeset = gpGlobals->time;  // KWo - 30.07.2006
               ...
               else
               {
                  ...
                  if ((iDestIndex!= pBot->curr_wpt_index))
                  {
                     ...
                     pBot->pWaypointNodes = pBot->pWayNodesStart;
//                   if (pBot->bot_team == 2)
//                      ALERT(at_logged, "[Debug] Continue following - Bot %s is following the player %s; target wp_index = %i.\n", pBot->name, STRING(pBot->pBotUser->v.netname), iDestIndex);
...
//                UTIL_ServerPrint ("[Debug] Bot %s is following the player %s; target wp_index = %i.\n", pBot->name, STRING(pBot->pBotUser->v.netname), iDestIndex);
               }
            }
...
//            ALERT(at_logged,"[DEBUG] Bot %s has started the TASK_PICKUPITEM with pickup_type = %s.\n", pBot->name, g_ItemTypeNames[pBot->iPickupType]);
         }
...
//            ALERT(at_logged,"[DEBUG] Bot %s finished the TASK_PICKUPITEM because of time expiring.\n", pBot->name);
/*
            if (pBot->iPickupType == PICKUP_WEAPON)
            {
               int iId = pBot->current_weapon.iId;
               ...
               ALERT(at_logged,"[DEBUG] Bot %s was trying to pickup %s; has %s with %d ammo in clip.\n", pBot->name,
                  vmodelname, weapon_defs[iId].szClassname, pBot->m_rgAmmoInClip[iId]);
               ALERT(at_logged,"[DEBUG] Bot %s has primary weapon %s with %d ammo in clip and sec weapon %s with %d ammo in clip.\n",  pBot->name,
...
//               ALERT(at_logged,"[DEBUG] Bot %s is trying to pickup %s.\n", pBot->name, vmodelname);
               // Near to Weapon ?
               if (distance < 50)
               {
                  ...
                  if (iGroundIndex > 6)
                  {
//                     ALERT(at_logged,"[DEBUG] Bot %s is trying to pickup prim weapon %s.\n", pBot->name, vmodelname);
                     if (BotHasShield (pBot))
...
//                        if (pBot->current_weapon.iId != cs_weapon_select[iWeaponNum].iId)
                        ...
                        {
//                           ALERT(at_logged,"[DEBUG] Bot %s doesn't have selected a primary gun.\n", pBot->name);
...
//                           ALERT(at_logged,"[DEBUG] Bot %s drops its primary weapon.\n", pBot->name);
                        }
                     }
...
//                     ALERT(at_logged,"[DEBUG] Bot %s is trying to pickup sec weapon %s.; iWeaponNum = %d.\n", pBot->name, vmodelname, iWeaponNum);
                     if (iWeaponNum != 0)
                     {
//                        if (pBot->current_weapon.iId != cs_weapon_select[iWeaponNum].iId)
                        ...
                        {
                           FakeClientCommand (pEdict, "drop");
//                           ALERT(at_logged,"[DEBUG] Bot %s drops its pistol.\n", pBot->name);
                        }
                     }
                     else if (g_i_cv_maxweaponpickup > 0)
                        pBot->iNumWeaponPickups++;
...
//                  ALERT(at_logged, "[DEBUG] TASK_PICKUPITEM - Bot %s found a hostage and is trying to pickup it.\n", pBot->name);
                  // Bot faces hostage ?
...
void BotThink (bot_t *pBot)
{
   ...
   else if (pBot->bDead)
   {
      ...
      {
//         UTIL_ServerPrint("[DEBUG] Bot %s is going to reply to the player; bot's last chat time = %f, global = %f.\n", pBot->name, pBot->f_lastchattime, g_fLastChatTime);
         if (!BotRepliesToPlayer (pBot) && (g_fLastChatTime + 15.0 < gpGlobals->time)
            && (pBot->f_lastchattime + 10.0 < gpGlobals->time)) // KWo - 27.03.2010
         {
...
/*
   if ((pHostEdict))
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
      {
         ...
         {
            ALERT(at_logged,"[DEBUG] Bot %s has primary weapon %s with %d ammo in clip and sec weapon %s with %d ammo in clip; current weapon - %s.\n",
            ...
            ALERT(at_logged,"[DEBUG] Bot %s %s to fire; bot %s reloading; Shoot time = %.2f, Time = %.2f .\n",
...
/*
         ALERT(at_logged,"[Debug] Bot %s - Current WP index = %i, Goal index = %i , State = %i, Task = %i, AimFlags = %i.\n", pBot->name,
               pBot->curr_wpt_index, pBot->chosengoal_index, pBot->iStates, pBot->pTasks->iTask, pBot->iAimFlags);
      }
   }
*/
 ...
 //           pBot->f_move_speed = 60.0;    // KWo - 24.01.2012
      ...
      if ((pBot->cCollisionState == COLLISION_NOTDECIDED)
         && (pBot->f_move_speed >= 0.0) && (pBot->f_move_speed < 2.0)) // KWo - 08.03.2010
         pBot->f_move_speed = 2.0;


      if ((pBot->cCollisionState == COLLISION_NOTDECIDED) && (pBot->f_move_speed != 0.0)
         && (pBot->f_wpt_tim_str_chg + 0.5 > gpGlobals->time)) // KWo - 08.03.2010
      {
...
/*
   if (pHostEdict)
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && (pHostEdict->v.button & IN_MOVELEFT))
         ALERT (at_logged,"[DEBUG] Bot %s %s able to check the terrain.\n", pBot->name, pBot->bCheckTerrain ? "is" : "is not");
   }
*/
...
/*
         ...
         {
            if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
               ALERT(at_logged, "[DEBUG] Bot %s must avoid a grenade; cAvoidGrenade = %d ; time = %f.\n", pBot->name, (int)pBot->cAvoidGrenade, gpGlobals->time);
         }
*/
      }
   }
...
/*
   if ((pHostEdict))
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
      {
         ...
         ALERT(at_logged, "[DEBUG] Bot %s - Curr. WP = %i, Goal WP = %i, Dist. to dest = %d, State = %i, Task = %s, AimFlags = %i,\n"
                          "Coll St = %i, Coll moves = %i, Vec look at = {%d, %d, %d}, ChAimDirT = %.2f, Time = %.2f. \n",
         ...
         if ((pHostEdict->v.button & IN_RELOAD) || (0 < 1))
         {
            ALERT(at_logged, "[Debug] Bot %s - look. pos.: v_angle = [%.1f, %.1f], ideal_ang = [%.1f, %.1f], ang_dev = [%.1f, %.1f], FrTimeInt = %.2f, aim_speed [%.2f, %.2f].\n",
...
*/
   ...
   {
//      if (g_iFrameCounter == g_i_botthink_index)
//         UTIL_ServerPrint("[DEBUG] Bot %s has fReachWpDelay = %f.\n", pBot->name, fReachWpDelay);
      GetValidWaypoint (pBot);
...
/*
   if ((pHostEdict))
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
      {
         ALERT(at_logged, "[DEBUG] Bot %s , Dist. to dest = %d, State = %i, Task = %s, AimFlags = %i, speed = %d,\n"
                          "Coll St = %i, Coll moves = %s, Time = %.2f. \n",
                  ...
                  pBot->cCollisionState, g_CollideMoveNames[pBot->cCollideMoves[(int) pBot->cCollStateIndex]],
                  gpGlobals->time);
      }
   }
*/
...
/*
   ALERT (at_logged,"[DEBUG] Bot %s move speed %.0f, moved distance = %i, dist to dest = %i, wp index = %d, TASK = %s, time = %.2f.\n",
