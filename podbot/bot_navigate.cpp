...
//   ALERT(at_logged, "[Debug] Bot %s found a new waypoint %i.\n", pBot->name, select_index);
...
bool BotHeadTowardWaypoint (bot_t *pBot)
{
   ...
   {
//      UTIL_ServerPrint("[DEBUG] Bot %s has no waypoint from pathfinding.\n", pBot->name);
      return (FALSE);
   }

//   UTIL_ServerPrint("[DEBUG] Bot %s head toward waypoint.\n", pBot->name);
   pEdict = pBot->pEdict;
   ...
   if (pBot->pWaypointNodes != NULL)
   {
      ...
      {
//         UTIL_ServerPrint("[DEBUG] Bot %s checks the next waypoint.\n", pBot->name);
...
/*
                     if (pHostEdict)
                     {
                        if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                           ALERT(at_logged, "[DEBUG] Bot's %s agression = %f, fKills = %f.\n",
...
//                   ALERT(at_logged,"[DEBUG] The bot %s will camp because of the fkills...\n", pBot->name);
                     BotPushTask (pBot, &TempTask);
...
bool BotCantMoveForward (bot_t *pBot, Vector vNormal)  // KWo - 13.07.2006
{
   ...
   if ((tr.flFraction < 1.0) && (!FNullEnt(tr.pHit)))
   {
      if ((strncmp ("func_door", STRING (tr.pHit->v.classname), 9) == 0)
         ...
         return (FALSE);
//      ALERT(at_logged, "[Debug] Bot %s can't move forward(1).\n", pBot->name);
   ...
   {
//      ALERT(at_logged, "[Debug] Bot %s can't move forward(2).\n", pBot->name);
   ...
   if ((tr.flFraction < 1.0) && (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) != 0)
      && ((pBot->bot_team == TEAM_CS_TERRORIST) || !(FStrEq ("hostage_entity", STRING (tr.pHit->v.classname))))) // KWo - 13.01.2008
   {
//      ALERT(at_logged, "[Debug] Bot %s can't move forward(3).\n", pBot->name);
      return (TRUE); // bot's body will hit something
   }
   ...
   if (pEdict->v.flags & FL_DUCKING)
   {
      ...
      if ((tr.flFraction < 1.0) && (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) != 0)
         && ((pBot->bot_team == TEAM_CS_TERRORIST) || !(FStrEq ("hostage_entity", STRING (tr.pHit->v.classname))))) // KWo - 13.01.2008
      {
//         ALERT(at_logged, "[Debug] Bot %s can't move forward(4).\n", pBot->name);
         return (TRUE); // bot's body will hit something
      }
      ...
      if ((tr.flFraction < 1.0) && (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) != 0)
         && ((pBot->bot_team == TEAM_CS_TERRORIST) || !(FStrEq ("hostage_entity", STRING (tr.pHit->v.classname))))) // KWo - 13.01.2008
      {
//         ALERT(at_logged, "[Debug] Bot %s can't move forward(5).\n", pBot->name);
         return (TRUE); // bot's body will hit something
      }
   }
   else
   {
      ...
      if ((tr.flFraction < 1.0) && (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) != 0)
         && ((pBot->bot_team == TEAM_CS_TERRORIST) || !(FStrEq ("hostage_entity", STRING (tr.pHit->v.classname))))) // KWo - 13.01.2008
      {
//         ALERT(at_logged, "[Debug] Bot %s can't move forward(6).\n", pBot->name);
         return (TRUE); // bot's body will hit something
      }
      ...
      if ((tr.flFraction < 1.0) && (strncmp ("func_door", STRING (tr.pHit->v.classname), 9) != 0)
         && ((pBot->bot_team == TEAM_CS_TERRORIST) || !(FStrEq ("hostage_entity", STRING (tr.pHit->v.classname))))) // KWo - 13.01.2008
      {
//         ALERT(at_logged, "[Debug] Bot %s can't move forward(7).\n", pBot->name);
         return (TRUE); // bot's body will hit something
      }
   }
...
bool BotCantMoveBack (bot_t *pBot, Vector vNormal)  // KWo - 16.09.2006
{
   ...
   // trace from the bot's eyes straight forward...
   TRACE_HULL (v_src, v_back, dont_ignore_monsters, (pEdict->v.flags & FL_DUCKING) ? head_hull : human_hull, pEdict, &tr);  // KWo - 13.01.2008
...
// KWo - 09.04.2006
      ...
      g_iUpdGlExpState = 0;
//      UTIL_ServerPrint("[Debug] Global experience update executed in 4 steps.\n");
      break;
   }
   return;
}
...
//   ALERT(at_logged, "[DEBUG] Bot %s collects damage %d experience.\n", STRING(pVictimEdict->v.netname), iDamage);
...
//               else
                 ...
                 /* && (clients[i].pEdict->v.movetype == MOVETYPE_FLY) */)    // and is using the ladder already  KWo - 05.01.2008
               {
                  ...
                  if (bFoundGround) // KWo - 05.01.2008
                  {
                     ...
                     {
//                         ALERT(at_logged, "[DEBUG] Bot %s forced to go away from the ladder - WPTindex = %i, out index = %i...\n", pBot->name, BotGetSafeTask(pBot)->iData, iPreviousWpt);
...
//         UTIL_ServerPrint ("[Debug] Bot %s is near WP with lift flag - WP index %i, state = %i.\n", pBot->name, pBot->curr_wpt_index, pBot->iLiftUsageState);
         ...
         || (FStrEq(szClassname, "func_train"))) && !bLiftExistDoorClosed) // KWo - 14.10.2006
      {
            ...
            || (pBot->iLiftUsageState == LIFT_LOOKING_BUTTON_OUTSIDE)) && (tr.pHit->v.velocity.z == 0))
         {
            ...
            {
//               UTIL_ServerPrint ("[Debug] Bot %s found a lift - ENTINDEX %i.\n", pBot->name, ENTINDEX(pBot->pLift));
               pBot->pLift = tr.pHit;
         ...
         {
//            UTIL_ServerPrint ("[Debug] Bot %s reached target traveling by the lift.\n", pBot->name);
...
//         if ((g_iFrameCounter == 10) && (bNeedWait))
//            UTIL_ServerPrint ("[Debug] Bot %s is waiting for someone in the lift.\n", pBot->name);
         if (bNeedWait)
         {
            pBot->dest_origin = pBot->v_LiftTravelPos;
      ...
      {
//         if (g_iFrameCounter == 10)
//            UTIL_ServerPrint ("[Debug] Bot %s is trying to find a button triggering the lift (inside).\n", pBot->name);
...
//            if (g_iFrameCounter == 10)
//               UTIL_ServerPrint ("[Debug] Bot %s found a lift button inside the lift and is trying to reach it.\n", pBot->name);
               ...
               && (pBot->pLift->v.velocity.z == 0) && (pEdict->v.flags & FL_ONGROUND) && (flDot >=0.90))
            {
               ...
               BotPushTask (pBot, &TempTask);
//               UTIL_ServerPrint ("[Debug] Bot %s is pushing a lift button from inside.\n", pBot->name);
            }
//
//          else if ((g_iFrameCounter == 10) && (pBot->fButtonPushTime + 1.0 < gpGlobals->time))
//          {
//             UTIL_ServerPrint ("[Debug] Bot %s can't reach the lift button from inside - distance %f.\n", pBot->name, (v_button_origin - pEdict->v.origin).Length ());
//          }
//
         }
      }
...
//            if (g_iFrameCounter == 10)
//               UTIL_ServerPrint ("[Debug] Bot %s is traveling with a lift.\n", pBot->name);
            if ((pEdict->v.origin - pBot->dest_origin).Length() < 15.0)
            {
               pBot->f_move_speed = 0.0;
               pBot->f_sidemove_speed = 0.0;
      ...
      if (pBot->iLiftUsageState == LIFT_LOOKING_BUTTON_OUTSIDE)
      {
         ...
         else
         {
            ...
            if (cNearestButtonIndex > -1)
            {
//               UTIL_ServerPrint ("[Debug] Bot %s found a lift button (outside).\n", pBot->name);
               for (i = 0; i < gpGlobals->maxClients; i++)
               {
                  if (!(clients[i].iFlags & CLIENT_USED)
                     || !(clients[i].iFlags & CLIENT_ALIVE)
                  ...
                  if (FStrEq(STRING(clients[i].pEdict->v.groundentity->v.targetname), ButtonsData[cNearestButtonIndex].target))
                  {
                     ...
                     {
//                        if (g_iFrameCounter == 10)
//                           UTIL_ServerPrint ("[Debug] Bot %s found somebody using already the lift.\n", pBot->name);
                        bLiftUsed = true;
                        break;
                     }
                  }
               ...
               else
               {
                  ...
                  if (((v_button_origin - pEdict->v.origin).Length () < 60.0) && (flDot>=0.90))
                  {
                     ...
                     bottask_t TempTask = {NULL, NULL, TASK_PAUSE, TASKPRI_PAUSE, -1, gpGlobals->time + 0.5, FALSE};
                     BotPushTask (pBot, &TempTask);
//                     UTIL_ServerPrint ("[Debug] Bot %s is pushing a lift button (outside).\n", pBot->name);
      ...
      {
//         if (g_iFrameCounter == 10)
//            UTIL_ServerPrint ("[Debug] Bot %s is waiting for a lift.\n", pBot->name);
         if ((pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))
         {
   ...
   if ((!FNullEnt(pBot->pLift)) && !(paths[pBot->curr_wpt_index]->flags & W_FL_LIFT))
   {
      if (pBot->iLiftUsageState == LIFT_TRAVELING_BY)
      {
//         UTIL_ServerPrint ("[Debug] Bot %s is leaving the lift.\n", pBot->name);
         pBot->iLiftUsageState = LIFT_LEAVING;
         pBot->f_UsageLiftTime = gpGlobals->time + 10.0;
      }
      ...
      {
//         UTIL_ServerPrint ("[Debug] Bot %s finished the travel by a lift.\n", pBot->name);
         pBot->iLiftUsageState = LIFT_NO_NEARBY;
         pBot->f_UsageLiftTime = 0.0;
         pBot->pLift = NULL;
      }
   ...
   {
//      UTIL_ServerPrint ("[Debug] Bot %s is no longer waiting for a lift - too long time; Lift state %i.\n", pBot->name, pBot->iLiftUsageState);
      pBot->pLift = NULL;
      pBot->iLiftUsageState = LIFT_NO_NEARBY;
      pBot->f_UsageLiftTime = 0.0;
      DeleteSearchNodes (pBot);
...
//         UTIL_ServerPrint ("[Debug] Bot %s found a button EntIndex %i .\n", pBot->name, EntHitIndex);
         if ((( (tr2.flFraction > 0.95) && (v_button_origin - pEdict->v.origin).Length () < 50.0)
            || ((EntHitIndex == ButtonsData[cNearestButtonIndex].EntIndex) && ((tr2.vecEndPos - GetGunPosition (pEdict)).Length() < 50.0))))
          {
            if (flDot > 0.92)
            {
//            UTIL_ServerPrint ("[Debug] Bot %s is using a button, task = %i, WP index = %i.\n", pBot->name, BotGetSafeTask(pBot)->iTask, pBot->curr_wpt_index);
               pEdict->v.button |= IN_USE;
   ...
   if (g_i_botthink_index == g_iFrameCounter ) // KWo - 23.03.2007
   {
      if ((pBot->bOnLadder) || (pBot->curr_travel_flags && W_FL_LADDER)) // 16.12.2007
         ...
         TRACE_HULL (v_trace_start, v_trace_end, ignore_monsters, (pEdict->v.flags & IN_DUCK) ? point_hull : head_hull, pEdict, &tr);
...
//                     cNearestButtonIndex = -1; // KWo - 16.07.2006
            ...
            {
//               UTIL_ServerPrint ("[Debug] Bot %s found a button.\n", pBot->name);
               v_button_origin = ButtonsData[cNearestButtonIndex].origin;
               ...
               v_src = v_button_origin;
               flDot = GetShootingConeDeviation (pEdict, &v_src);
//               UTIL_ServerPrint ("[Debug] Bot %s found a button targetting a door and faces it with flDot = %f .\n", pBot->name, flDot);
...
//                  pBot->iAimFlags &= ~AIM_ENTITY;
         ...
         if (!(FStrEq(vtargetname, "")) && (pBot->f_timeDoorOpen > gpGlobals->time) && bDoorHasTargetButton)  // KWo - 10.02.2006
         {
            if ((pBot->prev_wpt_index[0] >= 0) && (pBot->prev_wpt_index[0] < g_iNumWaypoints))
            {
//               UTIL_ServerPrint ("[Debug] Bot %s is trying to reach the previous WP .\n", pBot->name);
               pBot->dest_origin = paths[pBot->prev_wpt_index[0]]->origin;
               if ((pEdict->v.origin - pBot->dest_origin).Length() < 10.0)
               {
...
/*
            ...
            {
               if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                  ALERT(at_logged, "[Debug] Bot %s found a breakable (1) - faces with fCone = %f; LoopCheck = %i.\n", pBot->name, fCone, iAmountLooped);
            }
*/
         }
         else
...
/*
                  ...
                  {
                     if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                        ALERT(at_logged, "[Debug] Bot %s found a breakable (2) - faces with fCone = %f.\n", pBot->name, fCone);
                  }
*/
               }
            }
...
/*
         if (pHostEdict)
         {
            if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
...
*/
//   pBot->f_wpt_timeset = gpGlobals->time; // TEST !!!
   return;
}
...
/*
   if (pHostEdict)
   {
      if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && (pHostEdict->v.button & IN_MOVELEFT))
         ALERT(at_logged, "[Debug] Bot %s is checking blocking terrain.\n", pBot->name);
   }
*/
   ...
   {
      TRACE_HULL (v_src, v_dest, dont_ignore_monsters, ((pEdict->v.flags & IN_DUCK) ? point_hull : head_hull), pEdict, &tr);   // KWo - 08.02.2006
...
/*
               ...
               {
                  if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                     ALERT(at_logged, "[Debug] Bot %s found a breakable (3) - faces with fCone = %f.\n", pBot->name, fCone);
               }
*/
            }
         }
...
/*
                  ...
                  {
                     if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                        ALERT(at_logged, "[Debug] Bot %s found a breakable (4) - faces with fCone = %f.\n", pBot->name, fCone);
                  }
*/
               }
            }
...
/*
            ...
            {
               if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 20.0)
                  ALERT(at_logged, "[Debug] Bot %s found a breakable (5).\n", pBot->name);
            }
*/
         }
      }
...
/*
            if (pHostEdict)
            {
               if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
               {
                  ALERT (at_logged, "[Debug] Bot %s is about collision with %s, distance = %f, DotProductPos = %f, DotProductVel = %f, forward speed = %f.\n",
                     pBot->name, STRING(clients[index].pEdict->v.netname), f_nearestdistance, flDot1, flDot2, pBot->f_move_speed);
               }
            }
*/

            iBotBypassPlayerColl = BotCheckCollisionWithPlayer(pBot, pNearestPlayer);
//            UTIL_HostPrint ("Bot %s is close to %s; c_result = %d!\n", pBot->name, STRING(clients[index].pEdict->v.netname), iBotBypassPlayerColl);
...
//                        BotChangeWptIndex (pBot, pBot->pWaypointNodes->iIndex);
//                        pBot->wpt_origin = paths[pBot->curr_wpt_index]->origin;
//                        ALERT(at_logged, "[Debug] Bot %s is by-passing another player - current WP = %d, previous WP = %d.\n",
...
/*
               if (pHostEdict)
               {
                  if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
                  {
                     ALERT (at_logged, "[Debug] Bot %s is about collision with %s (distance = %f) and needs strafe %s.\n",
...
/*
            if (pHostEdict)
            {
               if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
               {
                  ALERT (at_logged,"[DEBUG] Bot %s %s stuck with player - %s; dist = %i, mov dist = %0.1f. NoCollTime %s gl_time, TASK = %s.\n",
                  ...
                  if (pHostEdict->v.button & IN_RELOAD)
                     ALERT (at_logged,"[DEBUG] Marking bot's stuck in the log.\n");
...
/*
      if (pHostEdict)
      {
         if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && (g_i_botthink_index % 4)== (g_iFrameCounter % 4))
         {
            ALERT (at_logged,"[DEBUG] Bot %s moved distance = %i, dist to dest = %i, speed = %.0f, TASK = %s, timers %s.\n",
...
/*
            ...
            {
               if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                  ALERT (at_logged,"[DEBUG] - NOT_DECID - Bot %s moved distance = %i, dist to dest = %i, speed = %.0f, TASK = %s.\n",
                     pBot->name, (int)pBot->f_moved_distance, (int)(pBot->pEdict->v.origin - pBot->dest_origin).Length(),
                     pBot->f_move_speed, g_TaskNames[iTask]);
            }
*/
...
//               pBot->prev_time = gpGlobals->time;
               bBotIsStuck = TRUE;
/*
               ALERT (at_logged,"[DEBUG] - NOT_DECID - Bot %s moved distance = %i, dist to dest = %i, speed = %.0f, TASK = %s, time = %.2f.\n",
                     pBot->name, (int)pBot->f_moved_distance, (int)(pBot->pEdict->v.origin - pBot->dest_origin).Length(),
...
/*
               ...
               {
                  if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                     ALERT (at_logged,"[DEBUG] Bot %s din't move enough or is stuck with another player.\n", pBot->name);
               }
*/
            }
...
/*
            if (pHostEdict)
            {
               if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
               {
                  ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s is stuck. Collision state = %i.\n", pBot->name, pBot->cCollisionState);
                  char szMessage[2048];
...
/*
                        if (pHostEdict)
                        {
                           if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && pBot->bPlayerCollision)
                              ALERT(at_logged, "[Debug] Bot %s needs obviate a player %s.\n", pBot->name, STRING(tr.pHit->v.netname));
                        }
*/
                     }
                  }
...
/*
                     ...
                     {
                        if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && (pHostEdict->v.button & IN_MOVELEFT))
                           ALERT(at_logged, "[Debug] Bot %s needs omit a hostage.\n", pBot->name);
                     }
*/
                  }
/*
                  else if (bCurrWptIndexOK)// KWo - 23.05.2010
                  {
                     v_dest = paths[pBot->curr_wpt_index]->origin;
                     TRACE_LINE (pEdict->v.origin, v_dest, dont_ignore_monsters, pEdict, &tr2);
                     if (tr2.flFraction >= 1.0)
                     {
                        v_trace_start = pEdict->v.origin - (pEdict->v.flags & IN_DUCK)? {0.0, 0.0, 15.0} : {0.0, 0.0, 30.0};
                        v_trace_end = pBot->dest_origin - (paths[pBot->curr_wpt_index]->flags & W_FL_CROUCH) ? {0.0, 0.0, 15.0} : {0.0, 0.0, 30.0};
                        TRACE_LINE (v_trace_start, v_dest, dont_ignore_monsters, pEdict, &tr2);
                        if (tr2.flFraction < 1.0)
                        {
                           a
*/
...
*/
                        ...
                        && !(pEdict->v.flags & FL_DUCKING)) // not blocked in the normal way, but maybe legs - needs overjump something?
                     {
                        ...
                        v_center.x = 0;
                        bTryJump = TRUE;
//                        ALERT(at_logged, "[DEBUG] Bot %s checks if can overjump some obstacle.\n", pBot->name);
                        // first do a trace from the bot's eyes forward...
                        v_src = GetGunPosition (pEdict); // EyePosition ()
...
/*
                              if ((pHostEdict) && bTryJump)
                              {
                                 if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                                    ALERT(at_logged, "[DEBUG] Bot %s wants to overjump some obstacle (1).\n", pBot->name);
                              }
*/
                           }
                        }
...
/*
                     ...
                     {
                        if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                        ALERT(at_logged, "[DEBUG] Bot %s is a leader and is too close to another player - wants jump.\n", pBot->name);
                     }
*/
                     bTryJump = TRUE;
                  }
...
/*
                     ...
                     {
                        if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                        ALERT(at_logged, "[DEBUG] Bot %s wants to overjump some obstacle (2).\n", pBot->name);
                     }
*/
                     cBits |= PROBE_JUMP;
                     cBits |= PROBE_DUCK;
...
/*
                           ...
                           {
                              if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                                 ALERT(at_logged, "[DEBUG] Bot %s wants to overjump some obstacle (3).\n", pBot->name);
                           }
*/
                           cBits |= PROBE_JUMP;
                        }
...
/*
                        ...
                        {
                           if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                              ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s has another player too close, wants to jump...\n",
                              pBot->name);
                        }
*/
                     }
               ...
               if (pHostEdict)
               {
                  if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0) && pBot->bPlayerCollision)
                     ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s is stuck, cBits = %s %s %s %s.\n",
                        pBot->name, (cBits & PROBE_JUMP) ? "PROBE_JUMP,":"", (cBits & PROBE_DUCK) ? "PROBE_DUCK,":"", (cBits & PROBE_STRAFE) ? "PROBE_STRAFE,":"", (cBits & PROBE_GOBACK) ? "PROBE_GOBACK":"");
               }
*/
...
/*
                     ...
                     {
                        if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                           ALERT (at_logged, "[Debug] Bot %s is stuck, Collide Move state = %i, weight = %i.\n", pBot->name, pBot->cCollideMoves[i], cState[i + 5]);
                     }
*/
                  }
...
/*
            if (pHostEdict)
            {
               if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
               {
                  ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s is stuck, incrementing cCollStateIndex = %d, time %.2f.\n",
                     pBot->name, pBot->cCollStateIndex, gpGlobals->time);
...
/*
                  ...
                  {
                     if ((pHostEdict->v.origin - pEdict->v.origin).Length() < 30.0)
                        ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s is stuck, incrementing cCollStateIndex = %d, speed = %d.\n",
                           pBot->name, pBot->cCollStateIndex, (int)pBot->f_move_speed);
                  }
*/
                  ...
                  if (pBot->cCollStateIndex > 5)  // KWo - 25.06.2006
                  {
//                     pBot->f_wpt_timeset = gpGlobals->time - 15.0; // KWo - 17.01.2010
                     BotResetCollideState (pBot);
                  }
               }
               if (pBot->cCollStateIndex <= 5)  // KWo - 25.06.2006
...
/*
                  if (pHostEdict)
                  {
                     if (((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0))
                     {
                        ALERT (at_logged, "[Debug] Bot %s is stuck, Collide Move = %i.\n", pBot->name, pBot->cCollideMoves[(int) pBot->cCollStateIndex]);

                        char szMessage[2048];
                        sprintf (szMessage,  "Bot %s is stuck. Collision Move = %i.\n", pBot->name, pBot->cCollideMoves[(int) pBot->cCollStateIndex]);
...
/*
                           ...
                           {
                              if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                                 ALERT (at_logged, "[Debug] Bot %s is stuck, trying to jump; time %f.\n", pBot->name, gpGlobals->time);
                           }
*/
                           pEdict->v.button |= IN_JUMP;
                           pBot->f_move_speed = pEdict->v.maxspeed;
...
/*
                        ...
                        {
                           if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                              ALERT (at_logged, "[Debug] Bot %s is stuck, trying to duck; time %f.\n", pBot->name, gpGlobals->time);
                        }
*/
...
/*
                        ...
                        {
                           if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                              ALERT (at_logged, "[Debug] Bot %s is stuck, trying to strafeleft; time %f.\n", pBot->name, gpGlobals->time);
                        }
*/
                        ...
                        if (!pBot->bBotNeedsObviateObstacle)
                        {
//                           ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes left - doesn't need omit an obstacle.\n", pBot->name);
...
//                                 ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes left (3).\n", pBot->name);
                        ...
                        {
//                           ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes left - needs omit an obstacle.\n", pBot->name);
...
/*
                        ...
                        {
                           if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                              ALERT (at_logged, "[Debug] Bot %s is stuck, trying to straferight; time %f.\n", pBot->name, gpGlobals->time);
                        }
*/
                        float fDot3;  // KWo - 09.01.2008
                        float fSideSpeed;  // KWo - 09.01.2008
                        ...
                        if (!pBot->bBotNeedsObviateObstacle)
                        {
//                           ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes right - doesn't need omit an obstacle.\n", pBot->name);
                           if (bPrevWptIndexOK && bCurrWptIndexOK)   // KWo - 14.01.2008
                           {
                              if ((pBot->f_timeHitDoor + 3.0 > gpGlobals->time) || (pBot->f_timeHitDoor < gpGlobals->time)) // KWo - 17.09.2006
                              {
                                 ...
                                 vec2DirToPrevWP = vec2DirToPrevWP.Normalize ();
//                                 if ((g_iFrameCounter/15) == int(g_iFrameCounter/15))
//                                    ALERT(at_logged, "[Debug] Bot %s is trying to straferight; DotProduct = %f.\n", pBot->name, DotProduct  (Vector2DirFromPoint, vec2DirToPrevWP));
                                 fDot3 = fabs(DotProduct  (Vector2DirFromPoint, vec2DirToPrevWP));  // KWo - 09.01.2008
                                 if (fDot3 < 0.99)
                                 {
                                    fSideSpeed = pEdict->v.maxspeed / 2.0;          // KWo - 09.01.2008
...
//                                 ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes right (3).\n", pBot->name);
                        ...
                        {
//                           ALERT(at_logged, "[DEBUG] BotCheckTerrain - Bot %s strafes right - needs omit an obstacle.\n", pBot->name);
//                           if (pBot->bPlayerCollision) // KWo - 27.02.2007
                           fStrafeDistance = (pBot->str_r_origin - pBot->pEdict->v.origin).Length();  // KWo - 09.01.2008
//                           pBot->dest_origin = pBot->str_r_origin;
                           pBot->f_move_speed = 0.0;
...
/*
                        ...
                        {
                           if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                              ALERT (at_logged, "[Debug] Bot %s is stuck, trying to go back; time %f.\n", pBot->name, gpGlobals->time);
                        }
*/
