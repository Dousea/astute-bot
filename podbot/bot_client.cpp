...
//                     ALERT(at_logged, "[DEBUG] CW1 - Player %s is reloading his weapon...\n", STRING(clients[bot_index].pEdict->v.netname));
                  }
               }
            }
...
/*
               if (pHostEdict)
               {
                  if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 30.0)
                     ALERT(at_logged,"[DEBUG] CurrentWeapon - Bot %s current weapon = %s, ammo in clip = %d. \n",
         ...
         if ((iId <= 31) && (iState == 1))
         {
            ...
            {                                       // - don't use that value
              ...
              if (iClip == 0) // KWo - 15.08.2007
              {
                 ...
                 {
                    clients[bot_index].fReloadingTime = gpGlobals->time + 1.5;
//                    ALERT(at_logged, "[DEBUG] CW2 - Player %s is reloading his weapon...\n", STRING(clients[bot_index].pEdict->v.netname));
                 }
              }
            }
...
/*
      if ((iId <= 31) && (iState == 1))
      {
         ALERT(at_logged,"[DEBUG] CurrentWeapon - Player %s, weapon = %s, WeaponState = %d, ammo in clip = %d. \n",
            bname, weapon_defs[iId].szClassname, iState, iClip);
      }
*/
   }
}
...
//               ALERT(at_logged,"[DEBUG] AmmoX - Bot %s current weapon = %s, ammo = %d, ammoId = %d.\n",
...
//               ALERT(at_logged, "[DEBUG] AmmoPickup - Bot %s weapon %s, current ammo %d, ammoId = %d.\n",
...
//      if (pHostEdict)
//      {
//         if ((pHostEdict->v.origin - pBot->pEdict->v.origin).Length() < 20.0)
...
//				ALERT(at_logged,"[DEBUG] Bot %s got infinite blinded.\n", pBot->name);
			}
         else
         {
